...
class UpdateTransferControllerApi {
  ...
  Future<Map<String, dynamic>?> postTransaction({
    ...
    required String currency,
    required String notes,
    String? imagePath, // اجعل الصورة اختيارية
  }) async {
    String url = "$baseUrl$id";
    String? token = await _getToken();
    if (token == null) {
    ...
    try {
      ...
      // تحقق من أن الصورة ليست رابطًا خارجيًا
      if (imagePath != null && imagePath.isNotEmpty && !imagePath.startsWith("http")) {
        print('Adding image from path: $imagePath'); // تحقق من المسار
        request.files.add(await http.MultipartFile.fromPath('image', imagePath));
      } else {
        print('Image path is either empty or already uploaded on the server.');
      }
